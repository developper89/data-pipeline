...
class CommandConsumer:
    ...
    async def _format_command_with_parser(self, device_id: str, command_data: dict) -> Optional[bytes]:
        ...
        try:
            ...
            # Get translator instance (same as normalizer)
    ...
    async def _get_translator_for_device(self, device_id: str):
        """Get translator instance for device (same logic as normalizer)."""
        cache_key = "default_protobuf"
        ...
        if cache_key not in self._translator_cache:
            try:
                ...
                # Find the first protobuf translator config
                protobuf_config = None
                for config_item in translator_configs:
                    if config_item.get('type') == 'protobuf':
                        protobuf_config = config_item
                        break

                if protobuf_config:
                    translator = TranslatorFactory.create_translator(protobuf_config)
                    self._translator_cache[cache_key] = translator
                else:
                    logger.error("No protobuf translator configuration found")
                    return None
