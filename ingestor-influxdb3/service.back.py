...
import config
from influx_writer import InfluxDB3Writer
...
logger = logging.getLogger("ingestor-influxdb3.service")

class IngestorService:
    """
    Core InfluxDB3 ingestor service implementation that:
    1. Consumes messages from Kafka
    2. Maps them to InfluxDB3 line protocol format
    3. Writes data to InfluxDB3 using HTTP API
    """
    
    def __init__(self):
        self.running = False
        self.consumer = None
        self.influx_writer = InfluxDB3Writer()
    ...
    async def initialize(self) -> bool:
        ...
        """
        logger.info("Initializing InfluxDB3 Ingestor Service")
        ...
        # Initialize InfluxDB3 writer
        if not await self.influx_writer.connect():
            logger.error("Failed to connect to InfluxDB3")
            return False
            
        # Ensure database exists
        if not await self.influx_writer.ensure_database_exists():
            logger.error("Failed to ensure InfluxDB3 database exists")
            return False
            
        logger.info("InfluxDB3 Ingestor Service initialized successfully")
        return True
    ...
    async def run(self):
        ...
        self.running = True
        logger.info("Starting InfluxDB3 Ingestor Service processing loop")
        ...
        logger.info("InfluxDB3 Ingestor Service processing loop stopped")
    ...
    async def _process_message(self, message) -> bool:
        ...
        try:
            ...
            should_persist = metadata.get("persist", True) if metadata else True  # Default to True if not specified
            ...
            # Map data to InfluxDB3 line protocol format
            line_protocol = self.data_mapper.map_data(validated_output, config.INFLUXDB3_TIMESTAMP_PRECISION)
            if not line_protocol:
                logger.warning(f"[{request_id}] No valid line protocol generated from message, skipping")
                return True  # Consider message as processed even if no line protocol was generated
                
            # Write line protocol to InfluxDB3
            success = await self.influx_writer.write_points(line_protocol)
            if success:
                logger.debug(f"[{request_id}] Successfully wrote data to InfluxDB3 for device {validated_output.device_id}")
                return True
            else:
                logger.error(f"[{request_id}] Failed to write data to InfluxDB3 for device {validated_output.device_id}")
    ...
    async def stop(self):
        ...
        logger.info("Stopping InfluxDB3 Ingestor Service")
        self.running = False
        self._stop_event.set()
        
        # Close InfluxDB3 connection
        logger.info("Closing InfluxDB3 connection")
        await self.influx_writer.close()
        
        # Consumer will be closed in the run loop's finally block
        logger.info("InfluxDB3 Ingestor Service stopped") 